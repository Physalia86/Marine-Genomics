...
Last of all we need some data to call. We will do this from a set of toy bam files I created for you earlier.

```shell
cd ~/align
cp /home/data/day2/align/*.bam* .
```
If this worked, you should have the bams and their indexes for 63 plaice! 

### Calling variants

To call variants, we will first use the `bcftools mpileup` tool to pileup our BAM files. What does this mean exactly? Well we will take all reads at a given position and call variants from the reads covering that position. We need to do this for all individuals.
...
```shell
bcftools view -h plaice.vcf.gz | head
bcftools view -h plaice.vcf.gz | tail
```
...
```shell
bcftools index plaice.vcf.gz
```

Let's see what variants are present at the start of chr2:

```shell
bcftools view -H -r chr2 plaice.vcf.gz | head
```
...
```shell
bcftools view -H -r chr2:100000-200000 plaice.vcf.gz
```
Now you're probably wondering what exactly all this data actually means. Let's explore in a bit more detail.

```shell
bcftools view -h plaice.vcf.gz | tail -1 | cut -f 1-9
bcftools view -H plaice.vcf.gz | head -1 | cut -f 1-9
```
...
```shell
bcftools view plaice.vcf.gz | grep -m 1 -A 1 "#CHROM" | cut -f 1-7
```
...
```shell
bcftools view -H plaice.vcf.gz | head -1 | cut -f 8
```
You can find out what these mean by grepping the header.

```shell
bcftools view -h plaice.vcf.gz | grep "INFO"
```
...
```shell
bcftools view -H plaice.vcf.gz | head -1 | cut -f 9
```

To investigate what these mean, grep the header again.

```shell
bcftools view -h plaice.vcf.gz | grep "##FORMAT"
```

Now let's take a look at the call for a single individual.

```shell
bcftools view -H plaice.vcf.gz | head | cut -f 20
```
