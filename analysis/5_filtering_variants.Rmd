...
cp /home/data/vcf/plaice_full.vcf.gz* ./
```
...
```shell
bcftools view -H plaice_full.vcf.gz | wc -l
```
...
```shell
bcftools view plaice_full.vcf.gz | vcfrandomsample -r 0.012 > plaice_subset.vcf
...
```shell
# compress vcf
bgzip plaice_subset.vcf
# index vcf
bcftools index plaice_subset.vcf.gz
```
...
```shell
SUBSET_VCF=~/vcf/plaice_subset.vcf.gz
OUT=~/vcftools/plaice_subset
...
``` r
var_qual <- read_delim("./plaice_subset.lqual", delim = "\t",
...
``` r
var_depth <- read_delim("./plaice_subset.ldepth.mean", delim = "\t",
...
``` r
var_miss <- read_delim("./plaice_subset.lmiss", delim = "\t",
...
Our plaice data has a very promising missingness profile - clearly most individuals have a call at almost every site. Indeed if we look at the summary of the data we can see this even more clearly.
...
``` r
var_freq <- read_delim("./plaice_subset.frq", delim = "\t",
...
``` r
ind_depth <- read_delim("./plaice_subset.idepth", delim = "\t",
...
``` r
ind_miss  <- read_delim("./plaice_subset.imiss", delim = "\t",
...
```r
ind_het <- read_delim("./plaice_subset.het", delim = "\t",
...
```shell
VCF_IN=~/vcf/plaice_full.vcf.gz
VCF_OUT=~/vcf/plaice_full_filtered.vcf.gz
```
...
```shell
cat out.log
bcftools view -H plaice_full_filtered.vcf.gz | wc -l
```
